...
import { QUERY_GET_INVIOCE_FREETOWN_ON_WAY } from '../Graphql/Query';
import Moment from 'react-moment';
import AsyncStorage from '@react-native-async-storage/async-storage';
import found from "../../assets/Image/nofound.jpg"
import moment from 'moment';

export default function FreetownOnWayBooking({ navigation }) {
    ...
    const { data, loading } = useQuery(QUERY_GET_INVIOCE_FREETOWN_ON_WAY, {
        variables: {
    ...
    useEffect(() => {
      if (data) {
        setFoundValue(data.getInvoiceByCollectionBoyFreetownIdonWay)
      }
    }, [data]);
  ...
  const filterByDate = (e) => {
    ...
    const keyword = e;
    if (keyword !== '') {
      const results = data.getInvoiceByCollectionBoyFreetownIdonWay.filter((data) => {
            ...
            <View style={{ backgroundColor: "#3498DB", height: 200, borderBottomLeftRadius: 30, borderBottomRightRadius: 30, flexDirection: "row", alignItems: "center", justifyContent: "center", width: "100%" }}>
                ...
                <View style={{ flexDirection: "column", justifyContent: "center", alignItems: "center", marginTop: 70 }}>
                    <View style={{ flexDirection: "row", width: "100%" }}>
                        ...
                        <Text style={{ fontSize: 20, fontFamily: "Poppins-SemiBold", color: "#fff", marginLeft: 10 }}>Pending</Text>
                    </View>
                </View>
            </View>
            {
                 data && data.getInvoiceByCollectionBoyFreetownIdonWay.length === 0 ?
                   <></>
                                            ...
                                            <Card style={{ width: "90%", height: 100, marginTop: 10, marginBottom: 10, elevation: 5, borderRadius: 10 }}>
                                                <View style={{ flexDirection: "row", width: "100%", justifyContent: "space-between" }}>
                                                    ...
                                                    <View style={{ width: "50%" }}>
                                                        ...
                                                        {
                                                            item.status === "On_Way" ?
                                                                <Text style={{ fontSize: 10, marginLeft: 10, fontFamily: "Poppins-SemiBold", color: "#F1C40F" }}>Pending</Text>
                                                                :
                                                                <></>
                                                        }
